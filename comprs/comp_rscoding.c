...
struct Polynomial *rs_correct_errata(struct Polynomial *msg_in, struct Polynomial * synd, struct Polynomial *err_pos, struct Tables *table, int bits)
{
    ...
    {
        long Xi_inv = gf_inverse_comp(*(X->poly_arr + i), table);
        // printf("X_inv: %ld\n", Xi_inv);
        ...
        // printf("y: %ld\n", y);
        y = gf_mul_comp(gf_pow_comp(*(X->poly_arr + i), 1, table), y, table);
        // printf("y: %ld\n", y);
        if (err_loc_prime == 0)
        {
    ...
    for (long i = 0; i < nsym - erase_count; i++)
    {
        ...
        if (delta != 0)
        {
            if (old_loc->poly_size > err_loc->poly_size)
            {
                ...
                old_loc = gf_poly_scale(err_loc, gf_inverse_comp(delta, table), table);
                err_loc = new_loc;
            }
